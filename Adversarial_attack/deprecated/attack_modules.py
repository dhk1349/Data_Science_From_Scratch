...
def deepfool(image, net, device='cpu', num_classes=1000, overshoot=0.02, max_iter=30):
    """
    things to fix
    detach function
    var names
...
def attack_test(model, device, test_loader, epsilon=None):
    ...
    for data, target in tqdm(test_loader):
        ...
        print(f"{init_pred} | {target.item()} | {final_pred.item()}")
        ...
        elif init_pred.item()==target.item() and final_pred.item()!=target.item():
            # print(f"{init_pred} | {target.item()} | {final_pred.item()}")
            # Save some adv examples for visualization later
            #if len(adv_examples) < 100:
            adv_ex = perturbed_data.squeeze().detach().cpu().numpy()
            adv_examples.append((init_pred.item(), final_pred.item(), adv_ex, data.squeeze().detach().cpu().numpy()))
            #np.save(f"./Imagenet_perturbed/deepfool/imgnet{cnt}.npy", np.array(init_pred.item(), final_pred.item(), adv_ex, data.squeeze().detach().cpu().numpy()))
        adv_ex = perturbed_data.squeeze().detach().cpu().numpy()
        adv_examples.append((init_pred.item(), final_pred.item(), adv_ex, data.squeeze().detach().cpu().numpy()))
        np.save(f"./Imagenet_perturbed/deepfool/imgnet{cnt}.npy",
                np.array((init_pred.item(), final_pred.item(), adv_ex, data.squeeze().detach().cpu().numpy())))
        if(len(adv_examples)==10):
            print(f"{len(adv_examples)} adv samples collected.. {type(adv_examples[0])}\nexiting..")
    ...
    return final_acc, np.array(adv_examples)
