...
if __name__ == '__main__':
    ...
    total = 0
    cnt = 0
    for data, target in tqdm(trainloader, desc="Testing"):
        ...
        cnt += (pred == target).sum().item()
        total += target.size(0)
        #print(f"{pred}, {target}")


    print(f"Training Acc: {cnt / total * 100:.2f}%\n")
    total = 0
    cnt = 0
    for data, target in tqdm(testloader, desc="testing"):
        # Send the data and label to the device
        data, target = data.to(device), target.to(device)
    ...
    print(f"Test Acc: {cnt / total * 100:.2f}%")